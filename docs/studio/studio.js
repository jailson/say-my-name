...
import { VARIANTS, VOICES } from './vendor/voices.js';
...
/** The eSpeak voice variant for this take. Not a component attribute — it only ever
    affects the audio generated here, which is then a plain file like any other. */
function variantOf(take) {
  return take.el.querySelector('.in-voice').value;
}
...
async function autoDerive(take) {
  ...
  try {
    const { ipa, wav } = await suggest(name, fields(take).lang, variantOf(take));
    ipaField.value = ipa;
    syncRespelling(take);
...
async function suggestFor(take) {
  ...
  try {
    ...
    rememberOptIn();
    markAutoMode();
    const voice = take.el.querySelector('.in-voice');
    const named = voice.value ? ` in ${voice.selectedOptions[0].textContent}'s voice` : '';
    takeReady(
      take,
      `Generated by eSpeak${named} — a guess. Edit the IPA, or record it yourself.`,
    );
  } catch (error) {
...
function addTake() {
  ...
  fillVoiceList();
  // Changing the voice only matters to audio this page generated: it re-speaks the same
  // phonemes in the new voice, and never touches a recording or an uploaded file.
  el.querySelector('.in-voice').addEventListener('change', () => {
    if (take.generated) void suggestFor(take);
  });
...
/**
 * Every voice eSpeak has, grouped by the gender it is labelled with upstream.
 *
 * These are variants of one synthesizer rather than separate voices — all robotic, but
 * audibly different, and the choice is baked into the file you download, so it works for
 * every visitor rather than depending on what their device happens to have installed.
 */
function fillVoiceList() {
  ...
  for (const select of document.querySelectorAll('.in-voice')) {
    if (select.dataset.filled) continue;
    select.dataset.filled = 'true';
    for (const [label, gender] of groups) {
      const matching = VARIANTS.filter((voice) => voice.gender === gender);
      if (!matching.length) continue;
      const group = document.createElement('optgroup');
      group.label = label;
      for (const { id, name } of matching) {
        const option = document.createElement('option');
        option.value = id;
