...
test.describe('studio', () => {
  ...
  test('picking a voice re-speaks the name in it', async ({ page }) => {
    await page.goto(STUDIO);
    const voice = page.locator('.in-voice').first();
    await expect(voice.locator('option')).not.toHaveCount(1); // the engine's variants loaded

    await page.locator('.suggest').first().click();
    await expect(page.locator('.rec-status').first()).toContainText('Generated by eSpeak', {
      timeout: 60_000,
    });
    const before = await page.locator('.size').first().textContent();

    await voice.selectOption('whisper');
    await expect(page.locator('.rec-status').first()).toContainText(/whisper's voice/i, {
      timeout: 60_000,
    });

    // Same name, same phonemes, different audio: a whisper is not the default voice.
    await expect(page.locator('.size').first()).not.toHaveText(before ?? '');
    await expect(page.locator('.in-synthetic').first()).toBeChecked();
  });

  test('a chosen voice never touches audio the page did not generate', async ({ page }) => {
    await page.goto(STUDIO);
    // An untouched take has no audio, so changing the voice must not invent any.
    await page.locator('.in-voice').first().selectOption('whisper');
    await page.waitForTimeout(500);
    await expect(page.locator('.rec-status').first()).toContainText('No audio yet');
    await expect(page.locator('.play').first()).toBeDisabled();
  });
